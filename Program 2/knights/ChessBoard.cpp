/* ChessBoard.cpp  -- Beth Allen Spring 2025
...
bool ChessBoard::findPath(int row, int col, int moveNumber) {
    ...
    if (inRange(row - 1, col + 2) && findPath(row - 1, col + 2, moveNumber + 1))  // R2, U1
        return true;
    if (inRange(row + 1, col + 2) && findPath(row + 1, col + 2, moveNumber + 1))  // R2, D1
        return true;
    if (inRange(row - 2, col + 1) && findPath(row - 2, col + 1, moveNumber + 1))  // R1, U2
        return true;
    if (inRange(row + 2, col + 1) && findPath(row + 2, col + 1, moveNumber + 1))  // R1, D2
        return true;
    if (inRange(row - 1, col - 2) && findPath(row - 1, col - 2, moveNumber + 1))  // L2, U1
        return true;
    if (inRange(row + 1, col - 2) && findPath(row + 1, col - 2, moveNumber + 1))  // L2, D1
        return true;
    if (inRange(row - 2, col - 1) && findPath(row - 2, col - 1, moveNumber + 1))  // L1, U2
        return true;
    if (inRange(row + 2, col - 1) && findPath(row + 2, col - 1, moveNumber + 1))  // L1, D2
        return true;
